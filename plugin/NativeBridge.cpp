        ...
        VariantMap& returnVarMap)
{
    ...
    //         keyUsages:   Array of strings from the set "encrypt", "decrypt",
    ...
    //         keyUsages:   Array of strings; same as input if present else empty
    //     }
    ...
    // get (optional) keyUsages; if not present leave empty
    bool isFound;
    VariantArray keyUsageVarAry = argsVar.mapValue<VariantArray>("keyUsages", &isFound);
    vector<CadmiumCrypto::KeyUsage> keyUsageVec;
    if (isFound)
        ...
        VariantMap& returnVarMap)
{
    ...
    // algorithm must have a params field
    if (!algObj.contains("params"))
    {
...
bool NativeBridge::derive(const string& cmdIndex, Variant& argsVar,
        VariantMap& returnVarMap)
{
    // return createKeyOp(  null,    null, algorithm, extractable, keyUsages, baseKeyHandle, derivedKeyAlgorithm, null)
    // var createKeyOp = (format, keyData, algorithm, extractable, keyUsages, baseKey, derivedKeyType,      key)
    ...
    //         keyUsages:   Array of strings; set to input keyUsages
    //     }
    ...
    // get (optional) dereived keyUsages; if not present leave empty
    bool isFound;
    VariantArray keyUsageVarAry = argsVar.mapValue<VariantArray>("keyUsages", &isFound);
    vector<CadmiumCrypto::KeyUsage> keyUsageVec;
    if (isFound)
        ...
        VariantMap& returnVarMap)
{
    ...
    //                        the wrapped key
    //         keyUsages:   Array of strings; resolved between input value and value
        ...
        VariantMap& returnVarMap)
{
    ...
    // var createKeyOp = (type,      format, keyData, algorithm,         extractable, keyUsages, baseKey,   derivedKeyType, key        )
...
bool NativeBridge::keyHandleToKeyVarMap(uint32_t keyHandle, VariantMap& keyVarMap)
{
    ...
    keyVarMap["keyUsages"]   = toKeyUsageVarAry(keyUsageVec);
    return true;
}

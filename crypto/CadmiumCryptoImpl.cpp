        ...
        const vector<KeyUsage>& keyUsage, uint32_t& keyHandle)
{
    ...
    if (jweHeaderAlgStr != "RSA-OAEP" && jweHeaderAlgStr != "A128KW")
    {
        DLOG() << "CadmiumCrypto::unwrapJwe: JWE Header alg field must be RSA-OAEP or A128KW, found "
                << jweHeaderAlgStr << endl;
        return CAD_ERR_UNKNOWN_ALGO; // TODO better error
    }
    const Algorithm jweHeaderAlg = (jweHeaderAlgStr == "A128KW") ? AES_KW : RSA_OAEP;
    ...
    if (wrappingKeyAlg == AES_KW && (extractIntFromString(jweHeaderEncStr) != (int)key.key.size() * 8))
    {
        // this should probably be an assert
